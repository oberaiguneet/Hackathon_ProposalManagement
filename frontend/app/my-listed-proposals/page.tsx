...
import { Bell, User, Paperclip, Calendar, MoreVertical, Edit, Trash, Copy, Check, Eye } from 'lucide-react'
...
export default function MyListedProposalsComponent() {
  ...
  const [proposals, setProposals] = useState([
    {
      ...
      tasks: [
        { id: 1, name: "Market Analysis", description: "Conduct a comprehensive market analysis of the retail sector.", assignedTo: "Alice Johnson", completed: true, attachments: ["market_analysis.pdf"] },
        { id: 2, name: "Technology Stack Selection", description: "Choose appropriate technologies for the digital transformation.", assignedTo: null, completed: false, attachments: [] },
        { id: 3, name: "Prototype Development", description: "Develop a prototype of the proposed digital solution.", assignedTo: "Bob Smith", completed: false, attachments: ["prototype_v1.zip"] },
        { id: 4, name: "User Testing", description: "Conduct user testing and gather feedback on the prototype.", assignedTo: null, completed: false, attachments: [] },
      ]
    },
  ...
  const cloneProposal = (id) => {
    const proposalToClone = proposals.find(proposal => proposal.id === id)
    if (proposalToClone) {
      const clonedProposal = {
        ...proposalToClone,
        id: proposals.length + 1,
        title: `${proposalToClone.title} (Clone)`,
        postingDate: new Date().toISOString().split('T')[0]
      }
      setProposals([...proposals, clonedProposal])
    }
  }
      ...
      <main className="flex-grow container mx-auto p-4 md:p-8">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-3/4">
            <h2 className="text-2xl font-bold mb-6">Your Listings for Proposal</h2>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => editProposal(proposal.id)}>
                            <Edit className="mr-2 h-4 w-4" />
                            <span>Edit</span>
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => cloneProposal(proposal.id)}>
                            <Copy className="mr-2 h-4 w-4" />
                            <span>Clone</span>
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => deleteProposal(proposal.id)}>
                          ...
                          <div className="flex justify-between items-center mt-2">
                            <Dialog>
                              <DialogTrigger asChild>
                                <Button variant="outline" size="sm" disabled={task.attachments.length === 0}>
